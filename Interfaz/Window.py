...
def abrirArchivo():
    ...
    path = filedialog.askopenfilename(title="Seleccione el archivo a analizar",
                                      filetypes=[("archivos de analisis", "*.html *.js *.css *.rmt")])
...
def mostrarRecorrido(listadoTokens):
    ...
    for i in listadoTokens:
        insercion += str(
            iterador) + ". Token: " + i.tipoToken + " Valor: " + i.valor + " Recorrido: " + i.recorrido + "\n"
...
def mostrarErrores(listadoErrores):
    ...
    for i in listadoErrores:
        insercion += str(iterador) + ". Fila: " + str(i.fila) + " Columna: " + str(
            i.columna) + " Error: " + i.error + "\n"
        consola.insert(INSERT, insercion)
        insercion = ""
        iterador += 1
